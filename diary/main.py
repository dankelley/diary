...
def diary():
    hints = [
            'see recent notes "diary -r"',
            'add a note: "diary -a" (opens EDITOR)',
            'add a note: "diary -a -t=title -c=content" (no EDITOR)',
            'add a note: "diary -a -t=title -c=content" -k=keywords"(no EDITOR)',
            'back up database by e.g. "cp ~/Dropbox/diary.db ~/diary-backup.db"',
            'create new book: "diary --create-book Bookname"',
            'create new note hashes: "diary --special rehash"',
            'create PDF of note with hash \'abcd\': "diary --markdown abcd | pandoc -V geometry:margin=1in -o abcd.pdf"',
            'delete note with hash \'ab...\': "diary -d ab"',
            'edit note with hash \'ab...\': "diary -e ab" (opens EDITOR)',
            'export all notes: "diary --export -" (import with \'--import\')',
            'export notes with hash \'ab...\': "diary --export ab"',
            'get help: "diary -h"',
            'import notes: "diary --import file.json" ("file.json" from "--export")',
            'list books: "diary --list-books"',
            'list keywords: "diary --list-keywords"',
            'list notes: "diary"',
            'list notes contained in the trash: "diary --trash"',
            'list notes due today: "diary --due today"',
            'list notes in markdown format: "diary --markdown"',
            'list notes with hash \'ab...\': "diary ab"',
            'list notes with keyword \'foo\': "diary -k foo"',
            'list notes within book: "diary -b Bookname"',
            'list notes without pager: "diary --pager=none"',
            'move note to new book: "diary --change-book hash Newbook"',
            'rename book: "diary --rename-book Old New"',
            'rename keyword: "diary --rename-keyword Old New"',
            'untrash notes with hash \'ab...\': "diary --undelete ab"',
            'extract attachments from note with given hash: "diary --extract hash"',
            'visit http://dankelley.github.io/diary/documentation.html to learn more']
    ...
    show_id = get_from_dotfile("~/.diaryrc", "show_id", False)
    debug = get_from_dotfile("~/.diaryrc", "debug", None)
    verbose = int(get_from_dotfile("~/.diaryrc", "verbose", -999))
    pager = get_from_dotfile("~/.diaryrc", "pager", None)

    parser = argparse.ArgumentParser(prog="diary", description="Diary: a diary tool",
            formatter_class=argparse.RawDescriptionHelpFormatter,
            epilog=textwrap.dedent('''\
    There are several ways to use diary. Try 'diary --hints' for some hints, and see
    http://dankelley.github.io/diary/ for more. Some common uses are as follows.

        diary                    # list notes, with first column being hash code
        diary ab                 # list notes with hash starting 'ab'
        diary -k key             # list notes with indicated keyword
        diary -a                 # add a note (opens a text editor)
        diary -a -t=... -c=...   # add a note (without a text editor)
        diary -e ab              # edit note with hash starting 'ab' (opens editor)
        diary -d ab              # delete note with hash starting 'ab'
        diary --export ab > F    # export note(s) to file 'F'
        diary --import F         # import note(s) from file 'F'
        diary --create-book Foo  # create a new book named Foo
        diary -b Foo             # list notes in book named Foo
        diary -r                 # list recent notes

    The ~/.diaryrc file may be used for customization, and may contain e.g. the
    following:
    ...
    parser.add_argument("--pager", type=str, dest="pager", default=None, help="pager for long output; may be 'more' (the default), 'less', or 'none'. It will be called with arguments '-R -X -F', which make sense for both 'less' and 'more'. If not given with --pager, a value is searched for in ~/.diaryrc.", metavar="cmd")
    ...
    defaultDatabase = get_from_dotfile("~/.diaryrc", "database", "~/Dropbox/diary.db")
    # Back to the parser
    parser.add_argument("--color", type=str, default=None, help="specify named scheme or True/False", metavar="c")
    parser.add_argument("--database", type=str, default=defaultDatabase, help="filename for database (defaults to ~/Dropbox/diary.db if not supplied as this argument, and if not specified in the ~/.diaryrc", metavar="db")
    ...
    # book_scheme = get_from_dotfile("~/.diaryrc", "book_scheme", 1)
    ...
    if args.color:
        ...
        color_scheme = get_from_dotfile("~/.diaryrc", "color", True)
    use_color = True
    ...
    if isinstance(color_scheme, str):
        ...
        if use_color:
            # color.hash = '\033[' + get_from_dotfile("~/.diaryrc", "color.hash", '31m')
            # color.title = '\033[' + get_from_dotfile("~/.diaryrc", "color.title", '1m')
            # color.keyword = '\033[' + get_from_dotfile("~/.diaryrc", "color.keyword", '35m')
            color.hash = color_code(get_from_dotfile("~/.diaryrc", "color.hash", 'red'))
            color.title = color_code(get_from_dotfile("~/.diaryrc", "color.title", 'bold'))
            color.keyword = color_code(get_from_dotfile("~/.diaryrc", "color.keyword", 'magenta'))
            color.book = color_code('blue') + color_code('bold')
    else:
        print("The color scheme given in the ~/.diaryrc file should be a string or logical")
        exit(1)
    ...
    diary = Diary(debug=args.debug, db=args.database, quiet=args.count)
    ...
    if args.create_book:
        diary.create_book(args.create_book)
        exit(0)

    if args.list_books:
        ''' List books. '''
        print("Books: ", end="")
        books = diary.list_books()
        nbooks = len(books)
    ...
    if args.change_book:
        ...
        diary.change_book(hash, book)
        exit(0)

    if args.rename_book:
        (old, new) = args.rename_book
        diary.rename_book(old, new)
        exit(0)

    if args.list_keywords:
        ''' List keywords. '''
        print("Keywords: ", end="")
        keywords = diary.list_keywords()
    ...
    if args.rename_keyword:
        ...
        diary.keyword_rename(old, new)
        exit(0)

    if args.special:
        if args.special == "rehash":
            diary.fyi("should rehash now")
            diary.rehash()
            sys.exit(0)
        else:
            diary.error("unknown action '%s'" % args.special)
    ...
    if args.book:
        b = diary.book_index(args.book)
        if len(b) > 1:
            diary.error("Abbreviation '%s' matches to %d books: %s" % (args.book, len(b), list(b.keys())))
        if not b:
            diary.error("No book named '%s'" % args.book)
        book = list(b.values())[0]
        diary.fyi("--book yields book index %s" % book)
    else:
        book = -1

    if args.delete:
        diary.fyi("should now delete note %s" % args.delete)
        diary.delete(args.delete)
        sys.exit(0)

    if args.undelete:
        diary.fyi("should now undelete note with hash %s" % args.undelete)
        diary.undelete(args.undelete)
        sys.exit(0)

    if args.empty_trash:
        diary.fyi("should now empty the trash")
        diary.empty_trash()
        sys.exit(0)

    if args.edit:
        diary.fyi("should now edit note %s" % args.edit)
        diary.edit(args.edit)
        sys.exit(0)
    ...
    if args.do_import: # need do_ in name to avoid language conflict
        try:
            ...
            diary.error("cannot read file '%s'" % args.do_import)
        notes = []
        i = 0
        for line in f:
            try:
                ...
                diary.error("cannot read line %d of file '%s'" % (line, args.do_import))
            i = i + 1
        for n in notes:
            try:
                # The 'book' is ignored because different users have different books.
                id = diary.add(title=n["title"], keywords=n['keywords'], content=n["content"], date=n['date'], due=n['due'])
            except:
                diary.error("cannot create note with title '%s'" % n["title"])

    if args.export:
        diary.fyi("should export now; hash=%s" % args.export)
        if args.export == '-':
            args.export = None
        noteIds = diary.find_by_hash(args.export)
        for n in noteIds:
    ...
    if args.trash:
        diary.fyi("should show trash contents now")
        ...
        if not '' == args.keywords[0]:
            trashed = diary.find_by_keyword(keywords=args.keywords, book=0)
        else:
            trashed = diary.find_by_hash(hash=args.hash, book=0)
        hal = diary.hash_abbreviation_length()
        for t in trashed:
    ...
    if args.add:
        if args.hash:
            diary.error("cannot specify a hash-code if the -a argument is given")
        # If no title is given, need to use the editor.
        if args.title == "":
            ee = diary.editor_entry(title=args.title, content=args.content, keywords=args.keywords, attachments=args.attachments, due=args.due, book=book)
            diary.add(title=ee["title"], keywords=ee["keywords"], content=ee["content"], book=ee["book"], due=ee["due"],
                    attachments=ee["attachments"])
        else:
            # FIXME: allow book below
            diary.add(title=args.title, keywords=args.keywords, content=args.content, due=args.due, book=book,
                    attachments=args.attachments)
        sys.exit(0)

    # By a process of elimination, we must be trying to find notes.
    due_requested = diary.interpret_time(args.due)
    if id_desired is not None:
        if id_desired[0:1] == '-': # don't get confused by arg flags
            id_desired = None
    trash_count = None
    if id_desired is not None:
        diary.fyi("search notes by hash (book=%s)" % book)
        found = diary.find_by_hash(hash=id_desired, book=book) # -1 means all books but trash
        trash_count = len(diary.find_by_hash(hash=id_desired, book=0))
    elif len(args.keywords[0]) and args.keywords[0] != '?':
        diary.fyi("search notes by keyword (book=%s)" % book)
        found = diary.find_by_keyword(keywords=args.keywords, book=book)
        trash_count = len(diary.find_by_keyword(keywords=args.keywords, book=0))
    elif args.recent_notes:
        if args.recent_notes == -2:
            found = diary.find_recent(nrecent=4)
        elif args.recent_notes == -1:
            found = diary.find_by_hash(hash=args.hash, book=book)
            trash_count = len(diary.find_by_hash(hash=args.hash, book=0))
        else:
            found = diary.find_recent(nrecent=args.recent_notes)
        trash_count = 0
    else:
        diary.fyi("Search notes by hashless method (book=%s)" % book)
        found = diary.find_by_hash(hash=args.hash, book=book)
        trash_count = len(diary.find_by_hash(hash=args.hash, book=0))
    count = 0
    nfound = len(found)
    ...
    for b in books_used:
        if not args.count and not args.due:
            if not args.pipe:
                if args.markdown:
                    print("Book: %s" % diary.book_name(b), end="\n\n")
                else:
                    print(color.book + "Book: %s" % diary.book_name(b) + color.normal, end="\n")
        for f in found:
            i = i + 1
            #print(f)
            try:
            ...
            if due_requested[0]:
                ...
                if when < 0:
                    continue
            count += 1 # FIXME: bug: 'diary --count' gives a huge number
            if not args.count:
                if nfound > 1:
                    ...
                    #print("id=%d"%f['noteId'])
                    #print("attachmentIds:")
                    attachmentIds = diary.get_attachment_list(noteId=f['noteId'])
                    ...
                    for attachmentId in attachmentIds:
                        ...
                        filename = diary.get_attachment_filename(attachmentId=attachmentId[0])[0]
                        #print("attachmentId %d" % attachmentId[0])
                        #echo "SELECT contents FROM attachment WHERE attachmentId=3;" | sqlite3 ~/Dropbox/diary.db
                        if args.extract_attachments:
                            #tmp = tempfile.NamedTemporaryFile(mode="wb", prefix="diary_", suffix="_"+str(filename[0]))
                            contents = diary.get_attachment_contents(attachmentId=attachmentId[0])
    ...
    if not args.count and args.verbose > 0 and not args.markdown:
        t = diary.trash_length()[0] # FIXME: should just return the [0]
        t = trash_count
        if t == 0:
            if nfound == 0:
